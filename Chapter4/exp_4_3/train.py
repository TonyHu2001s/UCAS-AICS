...
class COCODataSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        image = torch.from_numpy(image.astype(numpy.float32) / 255.0)
...
if __name__ == '__main__':
    ...
    s1_gram = get_gram_matrix(s1).detach()
    s2_gram = get_gram_matrix(s2).detach()
    s3_gram = get_gram_matrix(s3).detach()
    s4_gram = get_gram_matrix(s4).detach()
    j = 0
    count = 0
    epochs = 0
    while j <= epochs:
        for i, image in enumerate(data_loader):
            ...
            loss_s1 = loss_func(get_gram_matrix(out1), s1_gram)
            loss_s2 = loss_func(get_gram_matrix(out2), s2_gram)
            loss_s3 = loss_func(get_gram_matrix(out3), s3_gram)
            loss_s4 = loss_func(get_gram_matrix(out4), s4_gram)
